...
class IoO(Metric):
    """Computes intersection-over-output in images."""
    ...
    def update(self, output):
        ...
        output = activations.sum(dim=(-2, -1))
        # shape: (batch_size, n_labels)

        ioo = divide_tensors(intersection, output)
        # shape: (batch_size, n_labels)

        if gt_valid is not None:
            ioo = ioo * gt_valid # Only keep valid bboxes scores
            # shape: (batch_size, n_labels)
        ...
        added_ioo = ioo.sum(dim=0) # shape: n_labels

        if self._reduction:
            added_ioo = self._reduction(added_ioo) # shape: 1
            n_samples = self._reduction(n_samples) # shape: 1

        self._added_ioo += added_ioo
        self._n_samples += n_samples


    @sync_all_reduce('_n_samples', '_added_ioo')
    def compute(self):
        return divide_tensors(self._added_ioo, self._n_samples)
