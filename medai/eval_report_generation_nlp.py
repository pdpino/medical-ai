...
from medai.metrics import get_results_folder, save_results
...
def run_evaluation(run_name,
    ...
    reports_fpath = os.path.join(folder, f'outputs-{suffix}.csv')

    if not os.path.isfile(reports_fpath):
        print(f'File not found: {reports_fpath}')
        return

    df = pd.read_csv(reports_fpath)
