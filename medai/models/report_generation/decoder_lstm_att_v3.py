...
* Attented features are passed as input to the Linear layer that predicts the words,
    concatenated with the hidden state h
"""
...
class LSTMAttDecoderV3(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size,
        ...
        self.W_vocab = nn.Linear(hidden_size + features_size, vocab_size)
    ...
    def forward(self, image_features, reports=None, free=False, max_words=10000):
        ...
        for word_i in words_iterator:
            ...
            # Predict with FC
            fc_input = torch.cat((h_t, att_features), dim=1)
                # shape: batch_size, hidden_size + features_size
            prediction_t = self.W_vocab(fc_input)
